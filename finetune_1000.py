...
num_valid_samples = 52*50
data_dir = "/deepack/out_52_new_labels/"
save_dir = "/imagenet/model_tailoring/code/entropy/fine_tune_52C_52D/model"
# checkpoint = '/home/iitm/node2/imagenet/checkpoints/vgg_16.ckpt'
checkpoint = '/imagenet/models/slim/checkpoints/vgg_16.ckpt'
# checkpoint = '/imagenet/model_tailoring/code/entropy/scratch_vgg/model_scratch_vgg9985/model.ckpt'
dataset_split_name = "validation"                              # train/validation
...
def get_data_files(data_name):
    ...
    for k in range(length): # Train data tfrecords
        ...
        tfrecords_filename.append(data_dir+data_name+j+data)
...
entropy = pkl.load(open('/imagenet/model_tailoring/code/entropy/entropy.save','rb'))
mask = []
list_filters = [64,64,128,128,256,256,256,512,512,512,512,512,512]
prune_factor = [ 4, 2,  2,  2,  2,  2,  2,  2,  2,  2,  2,  2,512]
...
list_filenames = get_data_files(data_name = 'validation')
...
list_filenames_train = get_data_files(data_name = 'train')
...
#Inference
W_conv1_1_mask = tf.multiply(W_conv1_1,mask[0])
a_conv1_1 = tf.nn.bias_add(conv2d(images, W_conv1_1_mask),b_conv1_1)
h_conv1_1 = tf.nn.relu(a_conv1_1)
W_conv1_2_mask = tf.multiply(W_conv1_2,mask[1])
a_conv1_2 = tf.nn.bias_add(conv2d(h_conv1_1, W_conv1_2_mask),b_conv1_2)
h_pool1   = max_pool_2x2(tf.nn.relu(a_conv1_2))

W_conv2_1_mask = tf.multiply(W_conv2_1,mask[2])
a_conv2_1 = tf.nn.bias_add(conv2d(h_pool1, W_conv2_1_mask),b_conv2_1)
h_conv2_1 = tf.nn.relu(a_conv2_1)
W_conv2_2_mask = tf.multiply(W_conv2_2,mask[3])
a_conv2_2 = tf.nn.bias_add(conv2d(h_conv2_1, W_conv2_2_mask),b_conv2_2)
h_pool2   = max_pool_2x2(tf.nn.relu(a_conv2_2))

W_conv3_1_mask = tf.multiply(W_conv3_1,mask[4])
a_conv3_1 = tf.nn.bias_add(conv2d(h_pool2, W_conv3_1),b_conv3_1)
h_conv3_1 = tf.nn.relu(a_conv3_1)
W_conv3_2_mask = tf.multiply(W_conv3_2,mask[5])
a_conv3_2 = tf.nn.bias_add(conv2d(h_conv3_1, W_conv3_2),b_conv3_2)
h_conv3_2 = tf.nn.relu(a_conv3_2)
W_conv3_3_mask = tf.multiply(W_conv3_3,mask[6])
a_conv3_3 = tf.nn.bias_add(conv2d(h_conv3_2, W_conv3_3),b_conv3_3)
h_pool3   = max_pool_2x2(tf.nn.relu(a_conv3_3))

W_conv4_1_mask = tf.multiply(W_conv4_1,mask[7])
a_conv4_1 = tf.nn.bias_add(conv2d(h_pool3, W_conv4_1),b_conv4_1)
h_conv4_1 = tf.nn.relu(a_conv4_1)
W_conv4_2_mask = tf.multiply(W_conv4_2,mask[8])
a_conv4_2 = tf.nn.bias_add(conv2d(h_conv4_1, W_conv4_2),b_conv4_2)
h_conv4_2 = tf.nn.relu(a_conv4_2)
W_conv4_3_mask = tf.multiply(W_conv4_3,mask[9])
a_conv4_3 = tf.nn.bias_add(conv2d(h_conv4_2, W_conv4_3),b_conv4_3)
h_pool4   = max_pool_2x2(tf.nn.relu(a_conv4_3))

W_conv5_1_mask = tf.multiply(W_conv5_1,mask[10])
a_conv5_1 = tf.nn.bias_add(conv2d(h_pool4, W_conv5_1),b_conv5_1)
h_conv5_1 = tf.nn.relu(a_conv5_1)
W_conv5_2_mask = tf.multiply(W_conv5_2,mask[11])
a_conv5_2 = tf.nn.bias_add(conv2d(h_conv5_1, W_conv5_2),b_conv5_2)
h_conv5_2 = tf.nn.relu(a_conv5_2)
W_conv5_3_mask = tf.multiply(W_conv5_3,mask[12])
conv5_3 = tf.nn.bias_add(conv2d(h_conv5_2, W_conv5_3),b_conv5_3)
h_pool5   = max_pool_2x2(tf.nn.relu(conv5_3))
...
# optimize for fc network
# fc_vars = []
# for j in range(6,9):
#     fc_vars.append(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="vgg_16/fc"+str(j)+"/weights"))
#     fc_vars.append(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="vgg_16/fc"+str(j)+"/biases"))
...
with tf.Session(config=config) as sess:
    ...
    new_list_vars = []
    for e in list_vars:
    	if e not in (W_fc8, b_fc8):
    		new_list_vars.append(e)

    saver = tf.train.Saver(list_vars)
    saver_restore = tf.train.Saver(new_list_vars)
    saver_restore.restore(sess, checkpoint)
    coord = tf.train.Coordinator()
    threads = tf.train.start_queue_runners(coord=coord, sess = sess)
    steps = num_batches*num_epochs
    valid_acc = 0
    for k in range(int((52*50)/batch_size)):
...
# with open('test.save', 'wb') as pf:
#     # out_conv_12, out_labels, out_prediction = sess.run([conv_12, labels, prediction])
#     cp.dump([out_conv_12, out_labels, out_prediction], pf, cp.HIGHEST_PROTOCOL)
